...
const MODEL_ID = process.env.ARK_MODEL_ID || 'doubao-seed-1-6-251015';
...
app.post('/api/subtitle-suggestions', async (req, res) => {
  try {
    const { image_base64, image_url, max_tags = 6, language = 'zh' } = req.body || {};
    if (!image_base64 && !image_url) {
      console.log('subtitle-suggestions missing image payload', {
        has_base64: !!image_base64, has_url: !!image_url
      });
    ...
    console.log('Using imageUrl:', imageUrl ? (imageUrl.startsWith('data:') ? 'data:image/...' : imageUrl) : 'null');
    ...
    console.error('subtitle-suggestions error:', detail);
